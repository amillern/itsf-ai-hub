 ...
 *
 * Бере зібраний dist/, витягає розмітку <header> і <footer> з кожної сторінки,
 * нормалізує її та рахує SHA-256. Порівнює з еталонами в
 * .snapshots/layout.json. Будь-яка зміна — падіння збірки.
 *
 * Оновити еталон свідомо:  node scripts/check-layout-snapshots.mjs --update
 * (коміт із оновленим .snapshots/layout.json має рев'юїти людина —
 *  тримайте цей файл під CODEOWNERS)
 *
